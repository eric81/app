...
function tag(a, b, c) {
	for (var n = 1; n <= c; n++)
		n == a
				? (getObj("changebox" + b + n).className = "search1", getObj("produce" + b + n).style.display = "")
				: (getObj("produce" + b + n).style.display = "none", getObj("changebox" + b + n).className = "search2")
}
function refreshlog(a) {
	var b = "/comm/code.php?id=" + Math.random();
	a.src = b
}
var flag = !0;
function sublogin() {
	var a = $("#truename").val();
	if ("" == a)
		return $("#result").html("\u8bf7\u8f93\u5165\u7528\u6237\u540d"), !1;
	var b = $("#sfzhm").val();
	if ("" == b)
		return $("#result").html("\u8bf7\u8f93\u5165\u8bc1\u4ef6\u53f7\u7801"), !1;
	var c = $("#yzm").val();
	if ("" == c)
		return $("#result").html("\u8bf7\u8f93\u5165\u9a8c\u8bc1\u7801"), !1;
	$.ajax({
				type : "post",
				url : "/comm/logon.php",
				data : "truename=" + a + "&sfzhm=" + b + "&yzm=" + c,
				success : function(a) {
					"" == a ? window.location.reload() : "bcreg.php" == a
							? (flag = !0, window.location.href = "bcreg.php")
							: $("#result").html(a)
				}
			})
}
var isIE = document.all && window.ActiveXObject && !window.opera ? !0 : !1;
$(document).ready(function() {
	$(".tag_select").hover(function() {
				"tag_select" == this.className && (this.className = "tag_select_hover")
			}, function() {
				"tag_select_hover" == this.className && (this.className = "tag_select")
			}).click(function(a) {
		var b = this, c = b.id.slice(12);
		$(".tag_options").hide();
		"tag_select_hover" == $(this).attr("class") ? ($(this).attr("class", "tag_select_open"), $(this).next().css(
				"display", "")) : "tag_select_open" == $(this).attr("class")
				&& ($(this).attr("class", "tag_select_hover"), $(this).next().css("display", "none"));
		$(this).next().children().mouseover(function() {
					this.className = "open_hover"
				}).mouseout(function() {
					this.className = this.id == "selected_" + c ? "open_selected" : "open"
				}).click(function(a) {
					$("#selected_" + c).attr("class", "open").removeAttr("id");
					this.id = "selected_" + c;
					b.innerHTML = this.innerHTML;
					$(this).parent().hide();
					b.className = "tag_select"
				});
		isIE ? window.event.cancelBubble = !0 : a.stopPropagation()
	});
	$(document).click(function() {
				$(".tag_select_open").next().hide();
				$(".tag_select_open").attr("class", "tag_select")
			})
});
function getNavigatorType() {
	return "Microsoft Internet Explorer" == navigator.appName ? 1 : "Netscape" == navigator.appName ? 2 : 0
}
function getObject(a) {
	return document.getElementById && document.getElementById(a) ? document.getElementById(a) : document.all
			&& document.all(a) ? document.all(a) : document.layers && document.layers[a] ? document.layers[a] : !1
}
function wValChg(a, b) {
	"yyk" == a
			&& (getObject("wn_" + b + "").innerHTML = "\u533b\u9662", complete(hospitals), document
					.getElementById("words").name = "words", document.search.action = "/comm/list.php");
	"ksk" == a
			&& (getObject("wn_" + b + "").innerHTML = "\u79d1\u5ba4", complete(names), document.getElementById("words").name = "kesearch", document.search.action = "/comm/keSearchs.php");
	"jbk" == a
			&& (getObject("wn_" + b + "").innerHTML = "\u75be\u75c5", complete(diseases), document
					.getElementById("words").name = "keyword", document.search.action = "/comm/list/diseaseSearch.php");
	getObject("sbArea_" + b + "").style.display = "none"
}
function wValDisp(a, b) {
	"none" == getObject("sbArea_" + a + "").style.display
			? getObject("sbArea_" + a + "").style.display = ""
			: getObject("sbArea_" + a + "").style.display = "none"
}
function setSelBox(a) {
	switch (getNavigatorType()) {
		case 1 :
			a = window.event;
			node = a.srcElement;
			nodeName = a.srcElement.className;
			break;
		case 2 :
			node = a.target;
			nodeName = a.target.className;
			break;
		default :
			nodeName = "None"
	}
...
function complete(a) {
	$("#words").autocomplete(a, {
				...
				formatMatch : function(a, c, n) {
					return a.name + a.pinyin1 + a.pinyin2
				},
				formatResult : function(a) {
					return a.name
				}
			}).result(function(a, c, n) {
			})
}
function checkwords() {
...
function goTopEx() {
	...
	window.onscroll = function() {
		0 < document.documentElement.scrollTop ? a.style.display = "" : a.style.display = "none"
	};
	a.onclick = function() {
		var a = setInterval(function() {
					document.documentElement.scrollTop /= 1.1;
					1 > document.documentElement.scrollTop && clearInterval(a)
				}, 10)
	}
}
...
function createAjax() {
	...
	if (window.XMLHttpRequest)
		...
		for (var b = "MSXML2.XMLHTTP.5.0 MSXML2.XMLHTTP.4.0 MSXML2.XMLHTTP.3.0 MSXML2.XMLHTTP MICROSOFT.XMLHTTP.1.0 MICROSOFT.XMLHTTP.1 MICROSOFT.XMLHTTP"
				.split(" "), c = 0; c < b.length; c++)
			if (browserVersion.indexOf("MSIE 6.0")) {
				a = new ActiveXObject("MSXML2.XMLHTTP");
				break
			} else if (a = new ActiveXObject(b[c]))
				break;
	return a
}
function addFavorite() {
	if (document.all)
		try {
			window.external.addFavorite("http://www.bjguahao.gov.cn",
					"\u5317\u4eac\u5e02\u9884\u7ea6\u6302\u53f7\u7edf\u4e00\u5e73\u53f0")
		} catch (a) {
		...
		window.sidebar
				? window.sidebar.addPanel("\u5317\u4eac\u5e02\u9884\u7ea6\u6302\u53f7\u7edf\u4e00\u5e73\u53f0",
...
(function(a) {
	...
	a.Autocompleter = function(b, c) {
		function n() {
			...
			if (c.multiple) {
				var d = x(m.val());
				if (1 < d.length) {
					var p = c.multipleSeparator.length, l = a(b).selection().start, C, e = 0;
		...
		function s(a, b) {
			if (p == d.DEL)
				...
				var h = m.val();
				if (b || h != y)
					y = h, h = k(h), h.length >= c.minChars ? (m.addClass(c.loadingClass), c.matchCase
							|| (h = h.toLowerCase()), l(h, e, q)) : (m.removeClass(c.loadingClass), u.hide())
			}
		}
		function x(b) {
			return !b ? [""] : !c.multiple ? [a.trim(b)] : a.map(b.split(c.multipleSeparator), function(c) {
		...
		function q() {
			...
			c.mustMatch && m.search(function(a) {
						a
								|| (c.multiple ? (a = x(m.val()).slice(0, -1), m.val(a.join(c.multipleSeparator)
										+ (a.length ? c.multipleSeparator : ""))) : (m.val(""), m.trigger("result",
										null)))
					})
		}
		function e(f, g) {
			if (g && g.length && h) {
				...
				u.display(g, f);
				var e = g[0].value;
				c.autoFill && (k(m.val()).toLowerCase() == f.toLowerCase() && p != d.BACKSPACE)
						&& (m.val(m.val() + e.substring(k(y).length)), a(b).selection(y.length, y.length + e.length));
				u.show()
			} else
				q()
		}
		...
		var d = {
			...
			PAGEDOWN : 34,
			BACKSPACE : 8
		}, m = a(b).attr("autocomplete", "off").addClass(c.inputClass), t, y = "", v = a.Autocompleter.Cache(c), h = 0, p, r = {
			mouseDownOnSelect : !1
		}, u = a.Autocompleter.Select(c, b, n, r), w;
		a.browser.opera && a(b.form).bind("submit.autocomplete", function() {
					if (w)
						return w = !1
				});
		m.bind((a.browser.opera ? "keypress" : "keydown") + ".autocomplete", function(b) {
					h = 1;
					p = b.keyCode;
					switch (b.keyCode) {
						case d.UP :
						...
						case d.RETURN :
							if (n())
								return b.preventDefault(), w = !0, !1;
							break;
						case d.ESC :
							u.hide();
							break;
						default :
							clearTimeout(t), t = setTimeout(s, c.delay)
					}
				}).focus(function() {
					h++
				}).blur(function() {
					...
					function b(a, d) {
						...
						if (d && d.length)
							for (var e = 0; e < d.length; e++)
								if (d[e].result.toLowerCase() == a.toLowerCase()) {
									p = d[e];
									break
								}
					...
					a.each(x(m.val()), function(a, c) {
								l(c, b, b)
							})
				}).bind("flushCache", function() {
					v.flush()
				}).bind("setOptions", function(b, d) {
					a.extend(c, d);
	...
	a.Autocompleter.defaults = {
		...
		highlight : function(a, c) {
			return a.replace(RegExp("(?![^&;]+;)(?!<[^<>]*)(" + c.replace(/([\^\$\(\)\[\]\{\}\*\.\+\?\|\\])/gi, "\\$1")
									+ ")(?![^<>]*>)(?![^&;]+;)", "gi"), "<strong>$1</strong>")
		},
		scroll : !0,
		scrollHeight : 180
	...
	a.Autocompleter.Cache = function(b) {
		...
		function s() {
			...
			c[""] = [];
			for (var d = 0, m = b.data.length; d < m; d++) {
				var t = b.data[d], t = "string" == typeof t ? [t] : t, q = b.formatMatch(t, d + 1, b.data.length);
				if (!1 !== q) {
		...
		return {
			...
			load : function(e) {
				...
				if (k[e])
					return k[e];
				if (b.matchSubset)
					for (d = e.length - 1; d >= b.minChars; d--)
						if (m = k[e.substr(0, d)])
							return l = [], a.each(m, function(a, b) {
	...
	a.Autocompleter.Select = function(b, c, n, s) {
		function x() {
			y
					&& (v = a("<div/>").hide().addClass(b.resultsClass).css("position", "absolute")
							.appendTo(document.body), h = a("<ul/>").appendTo(v).mouseover(function(b) {
						k(b).nodeName && "LI" == k(b).nodeName.toUpperCase()
								&& (d = a("li", h).removeClass(e.ACTIVE).index(k(b)), a(k(b)).addClass(e.ACTIVE))
					}).click(function(b) {
		...
		function q(a) {
			...
			if (b.scroll) {
				...
				c + a[0].offsetHeight - h.scrollTop() > h[0].clientHeight ? h.scrollTop(c + a[0].offsetHeight
						- h.innerHeight()) : c < h.scrollTop() && h.scrollTop(c)
			}
		}
		var e = {
			ACTIVE : "ac_over"
		...
		return {
			display : function(c, n) {
				...
				t = n;
				h.empty();
				for (var q = b.max && b.max < m.length ? b.max : m.length, k = 0; k < q; k++)
					if (m[k]) {
						var f = b.formatItem(m[k].data, k + 1, q, m[k].value, t);
						!1 !== f
								&& (f = a("<li/>").html(b.highlight(f, t)).addClass(0 == k % 2 ? "ac_even" : "ac_odd")
										.appendTo(h)[0], a.data(f, "ac_data", m[k]))
					}
				l = h.find("li");
		...
		if (void 0 !== a)
			return this.each(function() {
						if (this.createTextRange) {
							...
							void 0 === c || a == c ? k.move("character", a) : (k.collapse(!0), k.moveStart("character",
									a), k.moveEnd("character", c));
							k.select()
						} else
							this.setSelectionRange ? this.setSelectionRange(a, c) : this.selectionStart
									&& (this.selectionStart = a, this.selectionEnd = c)
					});
		var n = this[0];
		if (n.createTextRange) {
			var s = document.selection.createRange(), x = n.value, k = s.text.length;
			s.text = "<->";
			s = n.value.indexOf("<->");
			n.value = x;
...
(function(a) {
	a.fn.KinSlideshow = function(b) {
		function c() {
			...
			a(p).append("<div class='KinSlideshow_titleBar'></div>");
			var c = b.titleBar.titleBar_height;
			isNaN(c) ? c = 40 : 25 > c && (c = 25);
			a(r + " .KinSlideshow_titleBar").css({
						height : c,
						width : "100%",
						position : "absolute",
						bottom : 0,
			...
			b.isHasTitleBar && a(r + " .KinSlideshow_titleBar").css({
						background : b.titleBar.titleBar_bgColor,
						opacity : b.titleBar.titleBar_alpha
					})
		}
		function n() {
			2 < b.btn.btn_borderWidth && (b.btn.btn_borderWidth = 2);
			if (0 > b.btn.btn_borderWidth || isNaN(b.btn.btn_borderWidth))
				b.btn.btn_borderWidth = 0;
			if (b.isHasBtn && 2 <= w) {
				a(p)
						.append("<div class='KinSlideshow_btnBox' style='position:absolute;right:10px;bottom:5px; z-index:100'></div>");
				var c = "";
				for (i = 1; i <= w; i++)
					c += "<li>" + i + "</li>";
				a(r + " .KinSlideshow_btnBox").append("<ul id='btnlistID' style='margin:0;padding:0; overflow:hidden'>"
						+ c + "</ul>");
				a(r + " .KinSlideshow_btnBox #btnlistID li").css({
							listStyle : "none",
							"float" : "left",
							width : 18,
							height : 18,
							borderWidth : b.btn.btn_borderWidth,
							borderColor : b.btn.btn_borderColor,
							borderStyle : "solid",
							background : b.btn.btn_bgColor,
							textAlign : "center",
							cursor : "pointer",
							marginLeft : 3,
							fontSize : 12,
							fontFamily : b.btn.btn_fontFamily,
							lineHeight : "18px",
							opacity : b.btn.btn_bgAlpha,
							color : b.btn.btn_fontColor
						});
				a("#btnlistID li:eq(0)").css({
							background : b.btn.btn_bgHoverColor,
							borderColor : b.btn.btn_borderHoverColor,
							color : b.btn.btn_fontHoverColor
						})
			}
		}
		function s() {
			a(r + " div:lt(2)").wrapAll("<div id='KinSlideshow_moveBox'></div>");
			a("#KinSlideshow_moveBox").css({
						width : f[0],
						height : f[1],
						overflow : "hidden",
						position : "relative"
					});
			a("#KinSlideshow_content").css({
						"float" : "left"
					});
			a("#KinSlideshow_contentClone").css({
						"float" : "left"
					});
			a(r + " #KinSlideshow_moveBox div").wrapAll("<div id='KinSlideshow_XposBox'></div>");
			a(r + " #KinSlideshow_XposBox").css({
						"float" : "left",
						width : "2000%"
					});
			B = setInterval(function() {
						e(b.moveStyle)
					}, 1E3 * z + b.moveSpeedTime)
		}
		function x() {
			a(r + " div:lt(2)").wrapAll("<div id='KinSlideshow_moveBox'></div>");
			a("#KinSlideshow_moveBox").css({
			...
			a("#KinSlideshow_contentClone").html("");
			a("#KinSlideshow_content a").wrap("<span></span>");
			a("#KinSlideshow_content a").each(function(b) {
						a("#KinSlideshow_contentClone").prepend(a("#KinSlideshow_content span:eq(" + b + ")").html())
					});
			a("#KinSlideshow_content").html(a("#KinSlideshow_contentClone").html());
			var c = (w - 1) * f[0];
			a("#KinSlideshow_moveBox").scrollLeft(c);
			B = setInterval(function() {
						e(b.moveStyle)
					}, 1E3 * z + b.moveSpeedTime)
		}
		function k() {
			...
			a("#KinSlideshow_moveBox").animate({
						scrollTop : 0
					}, 1);
			B = setInterval(function() {
						e(b.moveStyle)
		...
		function e(c) {
			switch (c) {
				case "left" :
					g >= w ? (g = 0, a("#KinSlideshow_moveBox").scrollLeft(0), a("#KinSlideshow_moveBox").animate({
								scrollLeft : f[0]
							}, b.moveSpeedTime)) : (sp = (g + 1) * f[0], a("#KinSlideshow_moveBox").is(":animated")
							&& a("#KinSlideshow_moveBox").stop(), a("#KinSlideshow_moveBox").animate({
								scrollLeft : sp
							}, b.moveSpeedTime));
					l(g + 1);
					break;
				case "right" :
					c = (w - 1) * f[0];
					g >= w
							? (g = 0, a("#KinSlideshow_moveBox").scrollLeft(c + f[0]), a("#KinSlideshow_moveBox")
									.animate({
												scrollLeft : c
											}, b.moveSpeedTime))
							: (A && (g++, A = !1), sp = c - g * f[0], a("#KinSlideshow_moveBox").is(":animated")
									&& a("#KinSlideshow_moveBox").stop(), a("#KinSlideshow_moveBox").animate({
										scrollLeft : sp
									}, b.moveSpeedTime));
					l(g);
					break;
				case "up" :
					g >= w ? (g = 0, a("#KinSlideshow_moveBox").scrollTop(0), a("#KinSlideshow_moveBox").animate({
								scrollTop : f[1]
							}, b.moveSpeedTime)) : (sp = (g + 1) * f[1], a("#KinSlideshow_moveBox").is(":animated")
							&& a("#KinSlideshow_moveBox").stop(), a("#KinSlideshow_moveBox").animate({
								scrollTop : sp
							}, b.moveSpeedTime));
					l(g + 1);
				...
				case "down" :
					c = (w - 1) * f[1], g >= w
							? (g = 0, a("#KinSlideshow_moveBox").scrollTop(c + f[1]), a("#KinSlideshow_moveBox")
									.animate({
												scrollTop : c
											}, b.moveSpeedTime))
							: (A && (g++, A = !1), sp = c - g * f[1], a("#KinSlideshow_moveBox").is(":animated")
									&& a("#KinSlideshow_moveBox").stop(), a("#KinSlideshow_moveBox").animate({
										scrollTop : sp
									}, b.moveSpeedTime)), l(g)
			}
		...
		function l(c) {
			c == w && (c = 0);
			b.isHasTitleFont && a(r + " .KinSlideshow_titleBar h2").html(u[c]);
			a("#btnlistID li").each(function(d) {
						d == c ? a(this).css({
									background : b.btn.btn_bgHoverColor,
									borderColor : b.btn.btn_borderHoverColor,
									color : b.btn.btn_fontHoverColor
		...
		function d() {
			a("#btnlistID li").mouseover(function() {
						var c = a("#btnlistID li").index(a(this));
						switch (b.moveStyle) {
							case "left" :
								g = c - 1;
								break;
		...
		for (h in t)
			void 0 == b.titleBar[h] && (b.titleBar[h] = t[h]);
		for (h in y)
			void 0 == b.titleFont[h] && (b.titleFont[h] = y[h]);
		for (h in v)
			void 0 == b.btn[h] && (b.btn[h] = v[h]);
		var p = this, r = a(p).selector, u = [], w = 0, f = [], g = 0, z = b.intervalTime, B, A = !0;
		if (isNaN(z) || 1 >= z)
			z = 5;
		500 < b.moveSpeedTime ? b.moveSpeedTime = 500 : 100 > b.moveSpeedTime && (b.moveSpeedTime = 100);
		a(p).css({
					visibility : "hidden"
				});
		a(r + " a img").css({
		...
		(function() {
			w = a(r + " a").length;
			f.push(a(r + " a img").width());
			f.push(a(r + " a img").height());
			a(r + " a img").each(function(b) {
						u.push(a(this).attr("alt"))
					});
			a(r + " a").wrapAll("<div id='KinSlideshow_content'></div>");
			a("#KinSlideshow_content").clone().attr("id", "KinSlideshow_contentClone").appendTo(p);
			c();
			b.isHasTitleFont
					&& (a(r + " .KinSlideshow_titleBar")
							.append("<h2 class='title' style='margin:3px 0 0 6px;padding:0;'></h2>"), a(r
							+ " .KinSlideshow_titleBar .title").css({
								fontSize : b.titleFont.TitleFont_size,
								color : b.titleFont.TitleFont_color,
								fontFamily : b.titleFont.TitleFont_family,
								fontWeight : b.titleFont.TitleFont_weight
							}), l(0));
			n();
			switch (b.moveStyle) {
				case "left" :
		...
		(function() {
			...
			a("#btnlistID li").mouseout(function() {
						B = setInterval(function() {
									e(b.moveStyle)
								}, 1E3 * z + b.moveSpeedTime)
					})
